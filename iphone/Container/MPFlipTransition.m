...
static CGFloat  rubberAmount = DEFAULT_RUBBERBAND_MAX_PROGRESS;

+ (void)setBounceFlipRatio:(CGFloat)value {
    NSLog(@"[INFO] init static bounce %f",value);
  rubberAmount = value;
}
...
- (id)initWithSourceView:(UIView *)sourceView destinationView:(UIView *)destinationView duration:(NSTimeInterval)duration style:(MPFlipStyle)style completionAction:(MPTransitionAction)action {
	...
	if (self)
	{
		...
		_stage = MPFlipAnimationStage1;
         NSLog(@"[INFO] use bounce %f",rubberAmount);

		_rubberbandMaximumProgress = rubberAmount;
...
- (void)buildLayers:(BOOL)isResizing
{
	...
	switch (self.completionAction)
	{
		...
		case MPTransitionActionShowHide:
			...
			isDestinationViewAbove = !isRubberbanding && [self.destinationView isAboveSiblingView:self.sourceView];
			break;
    ...
    // TODO when we are rubberbanding, we show nothing below right now. how can we differentiate when bouncing in the middle of the stack?
	...
	if (!isRubberbanding)
	{
		...
		if (drawReveal)
			[self.layerReveal setContents:(__bridge id)[pageRevealImage CGImage]];
		if (!isResizing)
	...
	if (drawFacing)
		[self.layerFacing setContents:(__bridge id)[pageFacingImage CGImage]];
	if (!isResizing)
	...
	[self.layerFront setContents:(__bridge id)[pageFrontImage CGImage]];
	if (!isResizing)
	...
	if (!isRubberbanding)
	{
		...
		[self.layerBack setContents:(__bridge id)[pageBackImage CGImage]];
	}
	...
	if (!isResizing)
	{
		...
		if (forwards)
			self.layerFrontShadow.colors = @[(__bridge id) [[[self flipShadowColor] colorWithAlphaComponent:0.5] CGColor], (id) [self flipShadowColor].CGColor, (__bridge id) [[UIColor clearColor] CGColor]];
		else
			self.layerFrontShadow.colors = @[(__bridge id) [[UIColor clearColor] CGColor], (id) [self flipShadowColor].CGColor, (__bridge id) [[[self flipShadowColor] colorWithAlphaComponent:0.5] CGColor]];
	...
	if (!isRubberbanding)
	{
		if (!isResizing)
		{
			...
			if (forwards)
				self.layerBackShadow.colors = @[(__bridge id) [[UIColor clearColor] CGColor], (id) [self flipShadowColor].CGColor, (__bridge id) [[[self flipShadowColor] colorWithAlphaComponent:0.5] CGColor]];
			else
				self.layerBackShadow.colors = @[(__bridge id) [[[self flipShadowColor] colorWithAlphaComponent:0.5] CGColor], (id) [self flipShadowColor].CGColor, (__bridge id) [[UIColor clearColor] CGColor]];
...
- (void)animateFlip1:(BOOL)isFallingBack fromProgress:(CGFloat)fromProgress toProgress:(CGFloat)toProgress withCompletion:(void (^)(BOOL finished))completion
{
    ...
    [animation setToValue:@(90 * factor * toProgress)];
    ...
    [animation setFromValue:@([self flippingPageShadowOpacity] * fromProgress)];
    [animation setToValue:@([self flippingPageShadowOpacity] * toProgress)];
	[animation setFillMode:kCAFillModeForwards];
	[animation setRemovedOnCompletion:NO];
...
- (void)animateFlip2:(BOOL)isFallingBack fromProgress:(CGFloat)fromProgress withCompletion:(void (^)(BOOL finished))completion
{
    ...
    if(fromProgress < 2.f)
    {
        ...
        [animation2 setFromValue:@([self flippingPageShadowOpacity] * (1 - fromProgress))];
        [animation2 setToValue:@0.0];
        [animation2 setFillMode:kCAFillModeForwards];
        [animation2 setRemovedOnCompletion:NO];
...
+ (void)dismissViewControllerFromPresentingController:(UIViewController *)presentingController duration:(NSTimeInterval)duration style:(MPFlipStyle)style completion:(void (^)(BOOL finished))completion
{
    ...
    UIViewController *dest = presentingController;
...
{
	UIViewController *toController = [self viewControllers][[[self viewControllers] count] - 2];
